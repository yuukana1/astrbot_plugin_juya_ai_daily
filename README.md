...
> 上述图片发送重试用于定时主动推送。手动 `/ainews` 每条命令最多调用一次图片发送；遇到异常时发送对应失败提示，不会补发第二张图片。`manual_dedupe_seconds` 只拦截同一个平台消息事件被重复处理，不会阻止新的手动命令，也不会与定时推送互相影响。

## 📝 指令列表

| 指令             | 说明                              |
| ---------------- | --------------------------------- |
| `/ainews`        | 立即获取最新一期完整图片日报 |
